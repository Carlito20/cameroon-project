...
async function printOrderReceipt(id) {
  ...
  const date = new Date((o.created_at || '') + ' UTC')
    .toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });

  let d = '';
  d += ESC + '@';
  d += ESC + 'E\x01';           // bold ON for entire receipt
  ...
  d += dashes() + LF;
  d += center(date) + LF;
  if (o.order_ref) d += center('Order: ' + o.order_ref) + LF;
  if (o.customer_name) d += center('Customer: ' + o.customer_name) + LF;
  if (o.customer_phone) d += center('Phone: ' + o.customer_phone) + LF;
  d += dashes() + LF;
  d += LF;
  ...
  items.forEach(item => {
    ...
    d += GS + '!\x00';
    d += ESC + 'E\x01';          // re-assert bold
    const meta = '  x' + qty + (price ? ' @ ' + price.toLocaleString() + ' FCFA' : '');
    const amt  = price ? fmt(line) : '--';
  ...
  d += dashes() + LF;
  d += GS + '!\x01';
  d += padLine('  TOTAL', fmt(o.total || total)) + LF;
  d += GS + '!\x00';
  d += ESC + 'E\x01';           // re-assert bold
  d += LF;
  d += padLine('Payment:', o.payment_method || 'N/A') + LF;
  if (o.payment_ref) d += padLine('Ref:', o.payment_ref) + LF;
  ...
  try {
    ...
    if (!resp.ok) throw new Error('relay error');
  } catch (e) {
    // Fallback to browser print
    const payIcon = (o.payment_method||'').includes('MTN') ? '🟡' : (o.payment_method||'').includes('Orange') ? '🟠' : '💵';
    const dateHtml = new Date((o.created_at||'') + ' UTC').toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' });
    let total2 = 0;
    ...
    document.getElementById('print-area').innerHTML = `
      <div style="text-align:center;margin-bottom:12px;">
        <h2 style="margin:0;font-size:20px;">American Select</h2>
        <p style="margin:2px 0;font-size:11px;color:#888;">americanselect.net</p>
      </div>
      ...
      <div style="display:flex;justify-content:space-between;font-size:18px;font-weight:bold;">
        <span>TOTAL</span><span>${(o.total || total2).toLocaleString()} FCFA</span>
      </div>
      <p style="margin-top:6px;font-size:12px;color:#888;">${payIcon} ${esc(o.payment_method || 'N/A')}</p>
      ${o.payment_ref ? `<p style="font-size:12px;color:#888;">Ref: <strong>${esc(o.payment_ref)}</strong></p>` : ''}
